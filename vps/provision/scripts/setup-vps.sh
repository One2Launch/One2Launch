...
sudo bash ./load-env-secrets

# funcs for printing messages
info() {
...
# set up an interrupt handler so user can exit gracefully
interrupt_count=0
interrupt_handler() {
  ((interrupt_count += 1))
...
# check for required tools before beginning
...
# get the min required version of Terraform
...
# get the curr version of Terraform
...
# check we meet the min required version
...
# check that we've already authenticated via Terraform in the static credentials file
...
applying your VPS infrastructure in less than a minute."
echo
...
# if there's an active trial, we should just retrieve that and configure
...
echo
echo "A workspace named '${workspace_name}' was created for you."
...
echo "  * Much more!"
echo
info "To see the infrastructure you just provisioned and continue exploring
HCP Terraform, visit:
